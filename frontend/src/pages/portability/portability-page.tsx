...
export function PortabilityPage() {
  ...
  const updateStatus = useMutation({
    mutationFn: ({ id, status }: { id: string; status: PortabilityStatus }) =>
      api.patch<{ data: Portability }>(`/portability/${id}`, { status }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['portability'] }),
  })
                ...
                <div className="flex gap-2">
                  {can('approve', 'portability') && (
                    <Button
                      ...
                      onClick={() => updateStatus.mutate({ id: info.row.original.id, status: 'approved' })}
                    >
                      Aprovar
                    </Button>
                  )}
                  {can('reject', 'portability') && (
                    <Button
                      ...
                      disabled={updateStatus.isPending}
                      onClick={() => updateStatus.mutate({ id: info.row.original.id, status: 'rejected' })}
                    >
                      Rejeitar
                    </Button>
                  )}
